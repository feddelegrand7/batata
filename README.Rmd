...
You can install the last R packages that you've starred on Github using the `install_starred_cran`. As an example, the following code chunk will look at the last 10 repositories that I've starred, filter R packages (I starr a lot of JS repos) and install them. 
...
The functions behave the same way as `install_starred_cran` except that it will install the repositories from Github (not from CRAN), so that you can install packages that are not available on CRAN. 
...
The above code chunk will look for the last five repos that I've starred, filter out non-R repositories and install R repos. 
